...
<body>
  ...
  it('should generate static HTML files for non-dynamic routes', () => {
    ...
    expect(fs.existsSync(path.join(testOutputDir, 'blog', ':id.html'))).toBe(false);
  });
  ...
  it('should apply correct meta tags to generated HTML', () => {
    ...
    // Check home page
    const homeHtml = fs.readFileSync(path.join(testOutputDir, 'index.html'), 'utf-8');
    expect(homeHtml).toContain('<title>Home</title>');
    expect(homeHtml).toContain('<meta name="description" content="Welcome to our site">');
    expect(homeHtml).toContain('<meta property="og:title" content="Home">');
    expect(homeHtml).toContain('<meta property="og:url" content="https://example.com/">');
    expect(homeHtml).toContain('data-route="/"');
    
    // Check about page
    const aboutHtml = fs.readFileSync(path.join(testOutputDir, 'about.html'), 'utf-8');
    expect(aboutHtml).toContain('<title>About Us</title>');
    expect(aboutHtml).toContain('<meta property="og:image" content="https://example.com/about.jpg">');
    expect(aboutHtml).toContain('data-route="/about"');
    
    // Check contact page with custom tags
    const contactHtml = fs.readFileSync(path.join(testOutputDir, 'contact.html'), 'utf-8');
    expect(contactHtml).toContain('<meta name="robots" content="noindex,follow">');
  });

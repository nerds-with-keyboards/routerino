...
describe('Social Media Preview Tags', () => {
  ...
  beforeEach(() => {
    ...
    // Clean up meta tags
    document.querySelectorAll('meta[property^="og:"], meta[name^="twitter:"], link[rel="canonical"]').forEach(tag => tag.remove());
  });
  ...
  describe('Open Graph Tags', () => {
    it('should set complete OG tags for social sharing', () => {
      ...
      expect(ogTitle.getAttribute('content')).toBe('Amazing Product | Example Site');
      expect(ogDescription.getAttribute('content')).toBe('The best product you will ever use');
      expect(ogImage.getAttribute('content')).toBe('https://example.com/product.jpg');
    });
    ...
    it('should use global imageUrl when route has none', () => {
      ...
      const ogImage = document.querySelector('meta[property="og:image"]');
      expect(ogImage.getAttribute('content')).toBe('https://example.com/default.jpg');
    });
  });
  ...
  describe('Canonical URLs', () => {
    it('should set canonical link tag', () => {
      const routes = [
        {
          ...
          tags: [
            { tag: 'link', rel: 'canonical', href: 'https://example.com/products/item-1' }
          ]
        }
      ];
      ...
      expect(canonical.getAttribute('href')).toBe('https://example.com/products/item-1');
    });
  });
  ...
  describe('Social Platform Specific', () => {
    it('should handle Facebook specific tags', () => {
      const routes = [
        {
          ...
          tags: [
            ...
            { property: 'article:publisher', content: 'https://facebook.com/example' }
          ]
        }
      ];
      ...
      expect(publisher.getAttribute('content')).toBe('https://facebook.com/example');
    });
  ...
  describe('Preview Validation', () => {
    it('should have all required tags for rich previews', () => {
      ...
      expect(requiredTags.description).toBe('This content looks great when shared');
    ...
    it('should handle special characters in meta content', () => {
      const routes = [
        {
          ...
          description: "Description with 'apostrophes' & special chars: < > \" '"
        }
      ];
      ...
      expect(ogDescription.getAttribute('content')).toBe(description.getAttribute('content'));
    });
  });
});
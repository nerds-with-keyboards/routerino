...
describe('Deprecation Warnings', () => {
  ...
  it('warns when both titlePrefix and titlePostfix are used', () => {
    ...
    render(<Routerino routes={routes} titlePrefix="PRE - " titlePostfix=" - POST" />)
    
    expect(warnSpy).toHaveBeenCalledTimes(2)
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("titlePrefix is deprecated"))
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("titlePostfix is deprecated"))
  })
  ...
  it('deprecated props still work correctly', () => {
    const routes = [
      { path: '/', element: <div>Home</div>, title: 'Page' }
    ]
    
    render(<Routerino routes={routes} titlePrefix="Site - " title="Fallback" />)

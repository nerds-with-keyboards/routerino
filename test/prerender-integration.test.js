...
const skipIntegration = process.env.CI === 'true' || process.env.SKIP_INTEGRATION !== 'false';
...
describe.skipIf(skipIntegration)('Prerender Server Integration', () => {
  ...
  describe('Prerendering', () => {
    ...
    it('should handle 404 pages correctly', async () => {
      const response = await fetch(`${PRERENDER_URL}/${TEST_APP_URL}/non-existent`, {
        headers: {
    ...
    it('should handle redirects', async () => {
      const response = await fetch(`${PRERENDER_URL}/${TEST_APP_URL}/redirect`, {
        headers: {
          'User-Agent': 'Googlebot'
        },
        redirect: 'manual'
      ...
      expect(response.headers.get('location')).toBe(`${TEST_APP_URL}/redirected`);
    });
  });
  ...
  describe('Caching', () => {
    it('should cache prerendered pages', async () => {
      ...
      const response = await fetch(`${PRERENDER_URL}/${TEST_APP_URL}/cached-test`, {
        headers: { 'User-Agent': 'Googlebot' }
      });

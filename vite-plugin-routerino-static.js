...
export function routerinoStatic(options = {}) {
  ...
  return {
    name: 'vite-plugin-routerino-static',
...
function applyMetaTags(html, route, baseUrl, globalMeta) {
  ...
  function updateHeadTagInHtml(html, { tag = 'meta', ...attrs }) {
    ...
    for (let i = 0; i < attrKeys.length; i++) {
      if (attrKeys[i] !== 'content') {
        ...
        const escapedValue = attrs[attrKeys[i]].replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        tagRegex = new RegExp(`<${tag}[^>]*${attrKeys[i]}=["']${escapedValue}["'][^>]*>`, 'i');
        if (tagRegex.test(html)) {
          // Update existing tag
          const newTag = `<${tag} ${attrKeys.map(key => `${key}="${escapeHtml(attrs[key])}"`).join(' ')}>`;
  ...
  if (description) {
    html = updateHeadTagInHtml(html, { name: 'description', content: description });
    html = updateHeadTagInHtml(html, { property: 'og:description', content: description });
  }
  ...
  if (baseUrl) {
    html = updateHeadTagInHtml(html, { property: 'og:url', content: `${baseUrl}${route.path}` });
    html = updateHeadTagInHtml(html, { tag: 'link', rel: 'canonical', href: `${baseUrl}${route.path}` });
  }

  // OG Image
  const imageUrl = route.imageUrl || globalMeta.imageUrl;
  if (imageUrl) {
    html = updateHeadTagInHtml(html, { property: 'og:image', content: imageUrl });
  }

  // Site name
  if (globalMeta.siteName) {
    html = updateHeadTagInHtml(html, { property: 'og:site_name', content: globalMeta.siteName });
  }

...
const config = {
  ...
  stripJsUserAgents: process.env.STRIP_JS_USER_AGENTS || DEFAULT_SEARCH_ENGINE_BOTS, // Who to strip JS for
...
console.log('Starting Routerino Prerender Server with config:', {
  ...
  prerenderUserAgents: config.prerenderUserAgents === 'all' ? 'all user agents' : 'specific pattern',
  stripJsUserAgents: config.stripJsUserAgents === 'none' ? 'keep JS for all' : (config.stripJsUserAgents === DEFAULT_SEARCH_ENGINE_BOTS ? 'strip JS for search engines only' : 'strip JS for custom pattern')
});
...
if (config.prerenderUserAgents !== 'all') {
  server.use({
    beforePhantomRequest: (req, res, next) => {
      ...
      const shouldPrerender = new RegExp(config.prerenderUserAgents, 'i').test(userAgent);
...
if (config.stripJsUserAgents !== 'none') {
  server.use({
    pageLoaded: (req, _res, next) => {
      const userAgent = req.headers['user-agent'] || '';
      const shouldStripJs = new RegExp(config.stripJsUserAgents, 'i').test(userAgent);
      ...
      if (shouldStripJs && req.prerender.content) {
        ...
        if (config.logRequests) {
          console.log(`[${new Date().toISOString()}] Stripped JS for user agent: ${userAgent}`);
        }
      }
  ...
  console.log('Script removal enabled for:', config.stripJsUserAgents === DEFAULT_SEARCH_ENGINE_BOTS ? 'search engines only' : 'custom pattern');
}
...
if (config.logRequests) {
  server.use({
    requestReceived: (req, _res, next) => {
      console.log(`[${new Date().toISOString()}] ${req.method} ${req.prerender.url}`);
      next();
    },
    pageLoaded: (req, _res, next) => {
      if (req.prerender.statusCode) {
        console.log(`[${new Date().toISOString()}] Rendered ${req.prerender.url} - Status: ${req.prerender.statusCode}`);
      }
      next();
    }
  });
...
console.log(`Prerender server listening on http://${config.host}:${config.port}`);
